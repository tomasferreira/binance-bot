...
- **EMA Crossover (50/200) – `ema_crossover`**: Uses two exponential moving averages \( \text{EMA}_{50}, \text{EMA}_{200} \) where \(\text{EMA}_n(t) = \alpha P_t + (1-\alpha)\text{EMA}_n(t-1)\), \(\alpha = 2/(n+1)\). A long signal is generated when \(\text{EMA}_{50}\) crosses from below to above \(\text{EMA}_{200}\) (golden cross) on closed candles and no position is open. Example: if on the previous candle \(\text{EMA}_{50}=29{,}800 < \text{EMA}_{200}=30{,}000\) and on the latest closed candle \(\text{EMA}_{50}=30{,}050 > \text{EMA}_{200}=30{,}010\), the strategy issues `enter-long`.
- **EMA Fast Crossover – `ema_fast_crossover`**: Same logic as the 50/200 crossover but with shorter lookbacks (e.g. 9/21 or 12/26 EMAs), making \(\alpha\) larger and the EMA more reactive. This catches earlier trend shifts at the cost of more false signals in choppy markets. Example: price whipsaw may cause several crosses of fast EMA over slow EMA within a small range, leading to more `enter-long`/`hold` churn unless filtered by regime.
- **Multi-EMA (9/21/50) – `multi_ema`**: Requires a bullish EMA “stack”: \(P > \text{EMA}_9 > \text{EMA}_{21} > \text{EMA}_{50}\). It also looks at the previous candle to detect when \(\text{EMA}_9\) crosses below \(\text{EMA}_{21}\) to exit. Example: if price = 31,000, \(\text{EMA}_9=30{,}950\), \(\text{EMA}_{21}=30{,}800\), \(\text{EMA}_{50}=30{,}500\) and no position is open, the stack condition is true and it issues `enter-long`; once \(\text{EMA}_9\) dips below \(\text{EMA}_{21}\) on a later candle, it issues `exit-long`.
- **Price vs EMA – `price_vs_ema`**: Compares the close \(P_t\) to a baseline EMA (e.g. \(\text{EMA}_{50}\)). When \(P_t / \text{EMA}_{50} - 1\) stays above a small positive threshold for several candles, it treats this as a persistent uptrend and may enter long; falls back below the EMA (ratio \(< 0\)) are used as exit/avoid conditions. Example: if price trades 1–2 % above \(\text{EMA}_{50}\) for 10+ bars, the strategy is biased to be long or stay long.
- **RSI Pullback – `rsi_pullback`**: Uses standard RSI on closes (with lookback \(n\); \(\text{RSI}=100-100/(1+\text{RS})\), \(\text{RS}=\text{avg gain}/\text{avg loss}\)). In an uptrend regime it looks for price above a trend filter (e.g. EMA) while RSI pulls back from overbought (>70) toward neutral (40–50) without overshooting into deep oversold. Example: after an up-leg where RSI peaked at 75, a dip back to ~45 with price still above the EMA can trigger `enter-long` as a “buy the dip” signal.
- **MACD Line Crossover – `macd`**: Computes \(\text{MACD} = \text{EMA}_{\text{fast}} - \text{EMA}_{\text{slow}}\) and a signal line \(\text{Signal} = \text{EMA}_9(\text{MACD})\). When MACD crosses from below to above the signal line on a closed candle (and regime is bullish), it issues `enter-long`. Example: previous candle MACD = −5, Signal = −3 (sub-signal), current candle MACD = 1, Signal = −0.5 → bullish zero-cross and line cross → long.
- **MACD Histogram Long – `macd_histogram_long`**: Focuses on the histogram \(H = \text{MACD} - \text{Signal}\). A long setup appears when \(H\) stops decreasing while still negative and then starts increasing toward and above 0 (bullish momentum inflection). Example: \(H\) sequence of −8, −5, −3, −1, +1 is interpreted as selling pressure exhausting then flipping to buying pressure; the first positive or strong uptick in \(H\) can trigger `enter-long`.
- **RSI + MACD Combo – `rsi_macd_combo`**: Requires both a bullish MACD condition (e.g. MACD > Signal, MACD >= 0) and a “healthy” RSI band (e.g. 40–65) to enter long, reducing trades when momentum and mean-reversion signals disagree. Example: MACD crosses up but RSI is already >80 → signal is ignored; MACD crosses up with RSI ~50 in an uptrend regime → `enter-long`.
- **Bollinger Mean Revert – `bollinger_mean_revert`**: Uses Bollinger Bands on close: middle band \(m_t\) is a moving average, upper/lower bands are \(m_t \pm k\sigma_t\), where \(\sigma_t\) is the rolling standard deviation. In ranging or low-volatility regimes, if price closes near or below the lower band (e.g. \(P_t < m_t - k\sigma_t\)), it treats this as an overshoot and looks to mean-revert back toward \(m_t\). Example: if \(m_t=30{,}000\), \(\sigma_t=200\), \(k=2\), lower band = 29,600 and close = 29,550, the strategy may issue `enter-long` with exits near the mid-band.
- **Bollinger Squeeze Breakout – `bollinger_squeeze`**: Measures band width \((\text{upper} - \text{lower})/m_t\) and compares it to a historical percentile; low width = squeeze. After a squeeze, a strong close beyond the upper band in a trending regime is treated as a volatility breakout and can trigger `enter-long`. Example: band width compresses to 1 % (10th percentile), then price closes above the upper band with expanding volume → the strategy enters long for a volatility expansion move.
- **Donchian Breakout – `donchian_breakout`**: Tracks the highest high and lowest low over a rolling window \(N\): \(H_N = \max(P_{\text{high},t-N+1..t})\), \(L_N = \min(P_{\text{low},t-N+1..t})\). A long breakout occurs when the close exceeds \(H_N\) by a small buffer, indicating a new \(N\)-bar high. Example: with \(N=20\) and previous 20-bar high = 31,200, a close at 31,350 can generate `enter-long`; exits often use stops below mid-channel or a new low breakout the other way.
- **Stochastic Oversold – `stochastic_oversold`**: Uses %K and %D lines where \(\%K_t = 100 \times (C_t - L_N)/(H_N - L_N)\) over \(N\) bars and \(\%D\) is a moving average of \%K. In bullish / ranging regimes it looks for \%K dipping below a level like 20 and then crossing back above \%D while price holds above a trend filter. Example: if \%K crosses up through \%D at 18 and price is still above the 50 EMA, it may issue `enter-long` as an oversold bounce.
- **ATR Trend – `atr_trend`**: Computes Average True Range (ATR) over \(N\) bars where True Range is \(\max(H_t - L_t, |H_t - C_{t-1}|, |L_t - C_{t-1}|)\). It then derives an ATR-based band under price (e.g. \( \text{ATR band} = C_t - m \cdot \text{ATR}_t\)). When price closes above a rising ATR band in a bullish regime and no position is open, it enters long; closes back through the band can trigger exit. Example: ATR = 150, multiplier \(m=2\), band = 30,000 − 300 = 29,700 and price closes at 30,200 → trend continuation long.
- **ATR Breakout – `atr_breakout`**: Uses ATR as a volatility filter for breakouts: requires that the move beyond a recent consolidation range is at least some multiple of ATR (e.g. breakout size \(\ge 1.5 \times \text{ATR}\)). Example: recent range high = 30,000, ATR = 100; a close at 30,250 (2.5 ATR above high) in a bullish regime can generate `enter-long`, while smaller one-ATR pokes are ignored.
- **Volume EMA Crossover – `volume_ema_crossover`**: Computes an EMA on volume \(V_t\) and compares current volume to \(\text{EMA}_V\). It typically only accepts long EMA-price crossovers when \(V_t > \text{EMA}_V \times k\) (e.g. 1.2×), signalling above-average participation. Example: price EMA crossover triggers but volume is 0.8× typical → signal skipped; later, another crossover with volume 1.5× EMA passes the filter and issues `enter-long`.
- **Multi-TF Trend – `multi_tf_trend`**: Combines higher-timeframe (HTF) and local-timeframe (LTF) indicators. Conceptually, it requires HTF trend filters (e.g. \(\text{EMA}_{200}^{\text{HTF}}\) slope or MACD > 0) to be bullish before acting on LTF triggers (e.g. EMA crossover). Example: 1h candles show price above 200 EMA and MACD > 0, while on 15m a bullish crossover occurs → `enter-long`; if HTF flips bearish, new LTF entries are blocked.
- **Range Bounce – `range_bounce`**: Estimates a horizontal trading range using recent swing highs/lows or a percentile of price distribution. In a ranging / low-vol regime, when price approaches the lower bound (e.g. within 5–10 % of range height) and shows stabilization, it enters long targeting the mid-range or upper bound. Example: range 29,500–30,500, height = 1,000; a close near 29,550 with no breakdown and neutral regime may generate `enter-long`, with exits around 30,000–30,300.

### Short-biased strategies

- **Short Trend (EMA 50/200) – `short_trend`**: Symmetric to the long EMA trend logic; looks for a bearish stack where \(P < \text{EMA}_{50} < \text{EMA}_{200}\). While this configuration holds and regime is bearish, it either enters or maintains a short. Exits occur when price closes back above \(\text{EMA}_{50}\) or when \(\text{EMA}_{50}\) is no longer below \(\text{EMA}_{200}\). Example: price = 29,000, \(\text{EMA}_{50}=29{,}200\), \(\text{EMA}_{200}=29{,}500\) → `enter-short`; later, price closing at 29,400 above \(\text{EMA}_{50}\) signals `exit-short`.
- **Short Breakdown – `short_breakdown`**: Uses support levels / recent lows similar to Donchian channels but inverted. If the close breaks below a recent multi-bar low \(L_N\) by more than a buffer and ATR confirms the move is large relative to noise, it issues `enter-short`. Example: 20-bar low = 28,500, ATR = 80; a close at 28,300 (2.5 ATR below low) in a bearish regime is treated as a downside breakout and shorted.
...
- **Short MACD – `short_macd`**: Mirror of the long MACD strategy; when MACD crosses below its signal line and is often below 0, it signals building downside momentum. Example: previous candle MACD = 3, Signal = 4; current MACD = 0, Signal = 2 (cross from above to below) in a bearish regime → `enter-short`.
...
- **Short Rejection – `short_rejection`**: Looks for failed upside breakouts or wick rejections near resistance / upper bands (EMAs, Bollinger, Donchian highs). Typical pattern: price spikes above a reference level intra-candle but closes back below it, creating a long upper wick and signaling rejection. Example: upper Bollinger band = 31,000; intrabar high = 31,200, close = 30,900 with a long upper wick in a bearish or neutral regime → `enter-short`.
