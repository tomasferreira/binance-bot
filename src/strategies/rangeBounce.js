...
export const description =
  'Mean reversion: long when price touches the lower bound of the recent range and bounces (close > open, near range low).'
...
export function evaluate (ohlcv, state) {
  const minLen = RANGE_LOOKBACK + BOUNCE_CANDLES + 2
  ...
  const longSignal = candleBullish && bouncedFromLow

  logger.info(
    `[${id}] price=${price.toFixed(2)} rangeLow=${rangeLow.toFixed(2)} rangeHigh=${rangeHigh.toFixed(2)} bullish=${candleBullish} bounced=${bouncedFromLow}`
  )

  if (longSignal) {
    logger.info(`[${id}] LONG signal (range bounce)`)
    return { action: 'enter-long', detail: { price, rangeHigh, rangeLow, rangeMid } }
  }
