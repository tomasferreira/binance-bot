import { calculateATR, calculateDonchian, calculateEMA, averageVolume } from '../indicators.js'
...
export function evaluate (ohlcv, state, context = {}) {
  ...
  if (log) {
    log.info(
      `[${id}] price=${price.toFixed(2)} donchianUpper=${upperPrev.toFixed(
        2
      )} breakAbove=${breakAbove} aboveEma50=${aboveEma50} volOk=${volOk}`
    )
  }

  if (!state?.openPosition && breakAbove && aboveEma50 && volOk) {
